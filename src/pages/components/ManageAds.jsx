...
const ManageAds = () => {
  ...
  const [adLocations, setAdLocations] = useState([
    ...
    { id: 'in-content', name: 'In-Content Ad', isVisible: true },
  ...
  const fetchAdVisibilitySettings = async () => {
    try {
      ...
      if (settings && Array.isArray(settings) && settings.length > 0) {
        ...
        console.log("Fetched and merged ad visibility settings:", mergedSettings);
      } else {
        // Fallback to default if API returns unexpected data
         const defaultLocations = [
          { id: 'header', name: 'Header Banner', isVisible: true },
          { id: 'left', name: 'Left Banner', isVisible: true },
          { id: 'right', name: 'Right Banner', isVisible: true },
      ...
      // setError('Failed to load ad visibility settings. Using defaults.');
    }
  };
  
  useEffect(() => {
    fetchAdVisibilitySettings();
    // The existing fetchData for vendors will also run
  }, []); // Run only once on mount

  // Handler for visibility change, passed to AdActivity
  const handleAdVisibilityChange = async (locationId) => {
    const newLocations = adLocations.map(location => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to update ad visibility settings');
      }
      
      // Update state only after successful API call
      setAdLocations(newLocations);
      console.log('Updated and saved ad visibility settings to API:', newLocations);
      // Optionally, show a success message
      // setSuccess('Ad visibility updated successfully!');
      // setTimeout(() => setSuccess(''), 3000);

    } catch (error) {
      console.error('Error updating ad visibility settings:', error);
      // Optionally, set an error state to inform the user
      setError('Failed to update ad visibility. Please try again.');
      // Consider reverting the optimistic UI update or re-fetching settings
    }
  };
